  ...
  x.reduce=x.foldl=x.inject=function(a,b,c,d){
    ...
    x.reduceRight=x.foldr=function(a,b,c,d){
      ...
      x.indexOf=function(a,b,c){
        ...
        var d=0,
        e=a.length;
        if(c){if("number"!=typeof c)
          return d=x.sortedIndex(a,b),a[d]===b?d:-1;d=0>c?Math.max(0,e+c):c
      }if(s&&a.indexOf===s)
      ...
      x.bindAll=function(a){
        ...
        y(b,function(b){a[b]=x.bind(a[b],a)}),a
      },
      ...
      x.debounce=function(a,b,c){
        ...
        x.wrap=function(a,b){
          return function(){
            var c=[a];return g.apply(c,arguments),b.apply(this,c)
          }
        },
        x.compose=function(){
          var a=arguments;
...
EPUBJS.Book.prototype.open=function(a,b){
  ...
  e&&!b&&this.applySavedSettings(),
  this.settings.contained||this.isContained(a)?(this.settings.contained=this.contained=!0,this.bookUrl="",c=this.unarchive(a).then(function(){return e&&d.settings.restore&&!b?d.restore():d.unpack()})):c=e&&this.settings.restore&&!b?this.restore():this.unpack(),this.online&&this.settings.storage&&!this.settings.contained&&(this.settings.stored||c.then(d.storeOffline())),c.then(function(){d.opened.resolve()}),c
},

EPUBJS.Book.prototype.unpack=function(a){
  var b=this,
  c=new EPUBJS.Parser,
  a=a||"META-INF/container.xml";
  return b.loadXml(b.bookUrl+a).then(function(a){
    return c.container(a)}).then(function(a){
      return b.settings.contentsPath=b.bookUrl+a.basePath,b.settings.packageUrl=b.bookUrl+a.packagePath,b.loadXml(b.settings.packageUrl)
    }).then(function(a){
      return c.package(a,b.settings.contentsPath)
    }).then(function(a){
      return b.contents=a,
      ...
      b.cover=b.contents.cover=b.settings.contentsPath+a.coverPath,
      b.spineNodeIndex=b.contents.spineNodeIndex=a.spineNodeIndex,
      ...
      b.settings.tocUrl=b.settings.contentsPath+a.tocPath,
      b.loadXml(b.settings.tocUrl)
    }).then(function(a){
      return c.toc(a)
    }).then(function(a){
...
EPUBJS.Book.prototype.networkListeners=function(){
  var a=this;window.addEventListener("offline",function(){
    a.online=!1,a.trigger("book:offline")
  },!1),window.addEventListener("online",function(){
    a.online=!0,a.trigger("book:online")
  },!1)
},

EPUBJS.Book.prototype.loadXml=function(a){
  return this.settings.fromStorage?this.storage.getXml(a):this.settings.contained?this.zip.getXml(a):EPUBJS.core.request(a,"xml")
},
...
EPUBJS.Book.prototype.urlFrom=function(a){
  ...
  c="/"==a[0],
  d=window.location,
  e=d.origin||d.protocol+"//"+d.host;
  return b?a:!b&&c?e+a:b||c?void 0:"../"==a.slice(0,3)?EPUBJS.core.resolveUrl(d.href,a):e+EPUBJS.core.folder(d.pathname)+a
},

EPUBJS.Book.prototype.unarchive=function(a){
  return this.zip=new EPUBJS.Unarchiver,this.zip.openZip(a)
},
...
EPUBJS.Book.prototype.isSaved=function(a){
  var b=a+":"+this.settings.version,
  c=localStorage.getItem(b);
  return localStorage&&null!==c?!0:!1
},

EPUBJS.Book.prototype.removeSavedSettings=function(){
  var a=this.settings.bookPath+":"+this.settings.version;
  localStorage.removeItem(a),
  this.settings.stored=!1
},

EPUBJS.Book.prototype.applySavedSettings=function(){
  var a=this.settings.bookPath+":"+this.settings.version;
  return stored=JSON.parse(localStorage.getItem(a)),EPUBJS.VERSION!=stored.EPUBJSVERSION?!1:(this.settings=_.defaults(this.settings,stored),void 0)
},

EPUBJS.Book.prototype.saveSettings=function(){
  var a=this.settings.bookPath+":"+this.settings.version;
  this.render&&(this.settings.previousLocationCfi=this.render.currentLocationCfi),
  localStorage.setItem(a,JSON.stringify(this.settings))
},

EPUBJS.Book.prototype.saveContents=function(){
  var a=this.settings.bookPath+":contents:"+this.settings.version;
  localStorage.setItem(a,JSON.stringify(this.contents))
},

EPUBJS.Book.prototype.removeSavedContents=function(){
  var a=this.settings.bookPath+":contents:"+this.settings.version;localStorage.removeItem(a)
},

EPUBJS.Book.prototype.renderTo=function(a){
  console.log(a)
  var b,c=this;if(_.isElement(a))this.element=a;
  else{
    if("string"!=typeof a)return console.error("Not an Element"),
      void 0;this.element=EPUBJS.core.getEl(a)
  }
  return b=this.opened.then(function(){
    return c.render=new EPUBJS.Renderer(c),c.startDisplay()
  },
  function(a){
    console.error(a)
  }),
...
EPUBJS.Book.prototype.startDisplay=function(){
  var a;
  return a=this.settings.restore&&this.settings.previousLocationCfi?this.displayChapter(this.settings.previousLocationCfi):this.displayChapter(this.spinePos)
},

EPUBJS.Book.prototype.restore=function(a){
  var b=this,
  c=this.settings.bookPath+":contents:"+this.settings.version,
  d=new RSVP.Promise,
  e=["manifest","spine","metadata","cover","toc","spineNodeIndex","spineIndexByURL"],
  a=a||!1,
  f=localStorage.getItem(c);
  return this.settings.clearSaved&&(a=!0),a||"undefined"==f||"null"==f||(this.contents=JSON.parse(f),e.forEach(function(c){
    b[c]=b.contents[c],b[c]||(a=!0)
  })
  ),
  !a&&f&&this.contents&&this.settings.contentsPath?(this.ready.manifest.resolve(this.manifest),this.ready.spine.resolve(this.spine),this.ready.metadata.resolve(this.metadata),this.ready.cover.resolve(this.cover),this.ready.toc.resolve(this.toc),d.resolve(),d):this.open(this.settings.bookPath,!0)
},

EPUBJS.Book.prototype.displayChapter=function(a,b){
  var c,d,e,f=this;
  return this.render?(_.isNumber(a)?e=a:(d=new EPUBJS.EpubCFI(a),
    e=d.spinePos),configure_reader(),load_context(),set_number(),footnote(),keyboard_act(),
  e>=this.spine.length?!1:0>e?!1:(this.spinePos=e,
    this.chapter=new EPUBJS.Chapter(this.spine[e]),
    c=f.render.chapter(this.chapter),
    d?c.then(function(b){
      b.currentLocationCfi=a,
      b.gotoCfiFragment(d),configure_reader(),load_context(),set_number(),footnote(),keyboard_act()
    })
    :b&&c.then(function(a){
      a.gotoChapterEnd(),configure_reader(),load_context(),set_number(),footnote(),keyboard_act()
    }),
    this.settings.fromStorage||this.settings.contained||c.then(function(){
      f.preloadNextChapter(),configure_reader(),load_context(),set_number(),footnote(),keyboard_act()
    }),c)):!1;
},

EPUBJS.Book.prototype.nextPage=function(){
  var a;if(this.render)return a=this.render.nextPage(),a?void 0:this.nextChapter()
},

EPUBJS.Book.prototype.prevPage=function(){
  var a;
  if(this.render)
  return a=this.render.prevPage(),a?void 0:this.prevChapter()
},

EPUBJS.Book.prototype.nextChapter=function(){
  return this.spinePos++,this.displayChapter(this.spinePos),load_context(),keyboard_act(),is_bookmark();
},

EPUBJS.Book.prototype.prevChapter=function(){
  return this.spinePos--,
  this.displayChapter(this.spinePos,!0),load_context(),set_number(),keyboard_act(),
  setTimeout(function(){
        is_bookmark()
  },500);
},
...
EPUBJS.Book.prototype.goto=function(a){
  ...
  d=c[0],
  e=c[1]||!1,
  f=-1==d.search("://")?this.settings.contentsPath+d:d,
  g=this.spineIndexByURL[f];
  console.log(f);
  //console.log(this,this.spineIndexByURL[f])
  if(this.render)return d||(g=this.chapter.spinePos),
    "number"!=typeof g?!1:g==this.chapter.spinePos&&this.chapter?(e&&this.render.section(e),
      (new RSVP.Promise).resolve(this.currentChapter)):this.displayChapter(g).then(function(){
      e&&b.render.section(e)
    })
...
EPUBJS.Book.prototype.preloadNextChapter=function(){
  ...
  this.spinePos>=this.spine.length?!1:(next=new EPUBJS.Chapter(this.spine[this.spinePos+1]),EPUBJS.core.request(next.href),void 0)
},

EPUBJS.Book.prototype.storeOffline=function(){
  var a=this,b=_.values(this.manifest);
  return EPUBJS.storage.batch(b).then(function(){
...
EPUBJS.Book.prototype.setStyle=function(a,b,c){
  this.settings.styles[a]=b,
  this.render&&this.render.setStyle(a,b,c)
},

EPUBJS.Book.prototype.removeStyle=function(a){
  this.render&&this.render.removeStyle(a),
  delete this.settings.styles[a]
},

EPUBJS.Book.prototype.unload=function(){
  this.settings.restore&&(this.saveSettings(),this.saveContents()),this.trigger("book:unload")
},
...
EPUBJS.Book.prototype.getHooks=function(){var a,b=this;plugTypes=_.values(this.hooks);for(plugType in this.hooks)a=_.values(EPUBJS.Hooks[plugType]),a.forEach(function(a){b.registerHook(plugType,a)})
},
...
EPUBJS.Book.prototype.triggerHooks=function(a,b,c){function d(){f--,0>=f&&b&&b()}var e,f;return"undefined"==typeof this.hooks[a]?!1:(e=this.hooks[a],f=e.length,e.forEach(function(a){a(d,c)}),void 0)},RSVP.EventTarget.mixin(EPUBJS.Book.prototype),EPUBJS.Chapter=function(a){this.href=a.href,this.id=a.id,this.spinePos=a.index,this.properties=a.properties,this.linear=a.linear,this.pages=1
},

EPUBJS.Chapter.prototype.contents=function(a){return a?a.get(href):EPUBJS.core.request(href,"xml")
},

EPUBJS.Chapter.prototype.url=function(a){var b=new RSVP.Promise;return a?(this.tempUrl||(this.tempUrl=a.getUrl(this.href)),this.tempUrl):(b.resolve(this.href),b)
},
...
EPUBJS.Chapter.prototype.unload=function(a){
  this.tempUrl&&a&&(a.revokeUrl(this.tempUrl),this.tempUrl=!1)
};
var EPUBJS=EPUBJS||{};EPUBJS.core={},
...
EPUBJS.core.request=function(a,b){
  function c(){if(this.readyState===this.DONE)if(200===this.status||this.responseXML){
    var a;a="xml"==b?this.responseXML:"json"==b?JSON.parse(this.response):this.response,d.resolve(a)
  }else d.reject(this)
}
var d=new RSVP.Promise,
e=new XMLHttpRequest;
return e.open("GET",a),
e.onreadystatechange=c,
"blob"==b&&(e.responseType=b),
"json"==b&&e.setRequestHeader("Accept","application/json"),"xml"==b&&e.overrideMimeType("text/xml"),
e.send(),d
},

EPUBJS.core.toArray=function(a){
  var b=[];for(member in a){
    var c;a.hasOwnProperty(member)&&(c=a[member],c.ident=member,b.push(c))
  }return b
},
...
EPUBJS.core.dataURLToBlob=function(a){
  ...
  if(-1==a.indexOf(b)){
    var c=a.split(","),
    d=c[0].split(":")[1],
    e=c[1];return new Blob([e],{type:d})
  }
  for(var c=a.split(b),d=c[0].split(":")[1],e=window.atob(c[1]),f=e.length,g=new Uint8Array(f),h=0;f>h;++h)g[h]=e.charCodeAt(h);return new Blob([g],{type:d})
},
...
EPUBJS.core.addScripts=function(a,b,c){var d=a.length,e=0,f=function(){e++,d==e?b&&b():EPUBJS.core.loadScript(a[e],f,c)};EPUBJS.core.addScript(a[e],f,c)
},
...
EPUBJS.core.prefixed=function(a){var b=["Webkit","Moz","O","ms"],c=a[0].toUpperCase()+a.slice(1),d=b.length,e=0;if("undefined"!=typeof document.body.style[a])return a;for(;d>e;e++)if("undefined"!=typeof document.body.style[b[e]+c])return b[e]+c;return a
},
...
EPUBJS.EpubCFI.prototype.generateChapter=function(a,b,c){
  var b=parseInt(b),
  a=a+1,
  d="/"+a+"/";
  return d+=2*(b+1),
...
EPUBJS.EpubCFI.prototype.generateFragment=function(a,b){
  var c=this.pathTo(a),
  d=[];return b&&d.push(b),
  c.forEach(function(a){
    d.push(2*(a.index+1)),
    a.id&&"EPUBJS"!=a.id.slice(0,6)&&d.push("["+a.id+"]")
  }),
  d.join("/")
},

EPUBJS.EpubCFI.prototype.pathTo=function(a){
  for(var b,c=[];a&&null!==a.parentNode;)b=a.parentNode.children,c.unshift({id:a.id,tagName:a.tagName,index:b?Array.prototype.indexOf.call(b,a):0}),a=a.parentNode;return c
},
...
EPUBJS.Parser.prototype.findTocPath=function(a){
  var b=a.querySelector("item[media-type='application/x-dtbncx+xml']");
...
EPUBJS.Parser.prototype.metadata=function(a){
  ...
  b.layout=c.querySelectorText(a,"meta[property='rendition:orientation']"),
  b.orientation=c.querySelectorText(a,"meta[property='rendition:orientation']"),
  b.spread=c.querySelectorText(a,"meta[property='rendition:spread']"),b
},
...
EPUBJS.Parser.prototype.spine=function(a,b){
  ...
  return e.forEach(function(a,d){
    ...
    f={id:e,linear:a.getAttribute("linear")||"",properties:a.getAttribute("properties")||"",href:b[e].href,index:d};
    c.push(f)
  }),c
},

EPUBJS.Parser.prototype.toc=function(a){
  function b(a){
    var c,
    d=[],
    e=[],
    f=a.childNodes,
    g=Array.prototype.slice.call(f),
    h=g.length,
    i=h;
    //console.log(a);
    //console.log(c);
    if(0==h)
      return!1;
    for(;i--;)c=g[i],
      "navPoint"===c.nodeName&&e.push(c);
    return e.forEach(function(a){
      var c=a.getAttribute("id"),
      e=a.querySelector("content"),
      f=e.getAttribute("src"),
      g=(f.split("#"),a.querySelector("navLabel")),
      h=g.textContent?g.textContent:"",
      i=b(a);
      d.unshift({id:c,href:f,label:h,subitems:i})
    }),d
  }
  var c=a.querySelector("navMap");
  return b(c)
},

EPUBJS.Renderer=function(a){
  this.el=a.element,
  this.book=a,
  this.caches={},
  this.crossBrowserColumnCss(),
  this.epubcfi=new EPUBJS.EpubCFI,
  this.initialize(),this.listeners()
},

EPUBJS.Renderer.prototype.initialize=function(){
  this.iframe=document.createElement("iframe"),
  this.book.settings.width||this.book.settings.height?this.resizeIframe(!1,this.book.settings.width||this.el.clientWidth,this.book.settings.height||this.el.clientHeight):(this.resizeIframe(!1,this.el.clientWidth,this.el.clientHeight),
...
EPUBJS.Renderer.prototype.listeners=function(){
  ...
  window.addEventListener("resize",this.resized,!1),
  this.book.registerHook("beforeChapterDisplay",this.replaceLinks.bind(this),!0),
  this.determineStore()&&this.book.registerHook("beforeChapterDisplay",[EPUBJS.replace.head,EPUBJS.replace.resources,EPUBJS.replace.svg],!0)
},

EPUBJS.Renderer.prototype.chapter=function(a){
  var b=this,
  c=!1;
  return this.book.settings.contained&&(c=this.book.zip),
  ...
  this.pageIds={},
  this.leftPos=0,
  this.currentChapterCfi=this.epubcfi.generateChapter(this.book.spineNodeIndex,a.spinePos,a.id),
  this.visibileEl=!1,a.url(c).then(function(a){
    return b.setIframeSrc(a)
  })
},

EPUBJS.Renderer.prototype.onResized=function(){
  var a={width:this.el.clientWidth,height:this.el.clientHeight};
  this.trigger("renderer:resized",a),
  this.book.trigger("book:resized",a),
  this.reformat()
},

EPUBJS.Renderer.prototype.reformat=function(){
  var a=this;a.book.settings.fixedLayout?a.fixedLayout():a.formatSpread(),
  setTimeout(function(){
    a.calcPages(),
    a.currentLocationCfi&&a.gotoCfiFragment(a.currentLocationCfi)
  },10)
},

EPUBJS.Renderer.prototype.resizeIframe=function(a,b,c){
  var d,e;a?(d=a.width,e=a.height):(d=b,e=c),
  this.iframe.height=e,0!=d%2&&(d+=1),
  this.iframe.width=d
},
...
EPUBJS.Renderer.prototype.setIframeSrc=function(a){
  var b=this,
  c=new RSVP.Promise;
  return this.visible(!1),
  this.iframe.src=a,
  this.iframe.onload=function(){
    b.doc=b.iframe.contentDocument,
    b.docEl=b.doc.documentElement,
    b.bodyEl=b.doc.body,
    b.applyStyles(),
    b.book.settings.fixedLayout?b.fixedLayout():b.formatSpread(),
    b.beforeDisplay(function(){
      b.calcPages(),
      c.resolve(b),
      b.currentLocationCfi=b.getPageCfi(),
      b.trigger("renderer:chapterDisplayed",b.currentChapter),
      b.book.trigger("renderer:chapterDisplayed",b.currentChapter),
      b.visible(!0)
    })
  },c
},
...
EPUBJS.Renderer.prototype.formatSpread=function(){
  var a=2,
  b=800;
  this.colWidth&&(this.OldcolWidth=this.colWidth,this.OldspreadWidth=this.spreadWidth),
  this.elWidth=this.iframe.width,this.gap=this.gap||Math.ceil(this.elWidth/8),this.elWidth<b||!this.book.settings.spreads?(this.spread=!1,a=1,this.colWidth=Math.floor(this.elWidth/a)):(this.spread=!0,this.colWidth=Math.floor((this.elWidth-this.gap)/a)),this.spreadWidth=(this.colWidth+this.gap)*a,
...
EPUBJS.Renderer.prototype.applyStyles=function(){
  var a=this.book.settings.styles;
  for(style in a)this.setStyle(style,a[style])
},
...
EPUBJS.Renderer.prototype.visible=function(a){
  return"undefined"==typeof a?this.iframe.style.visibility:(1==a?this.iframe.style.visibility="visible":0==a&&(this.iframe.style.visibility="hidden"),void 0)
},
...
EPUBJS.Renderer.prototype.nextPage=function(){
  return this.chapterPos<this.displayedPages?(this.chapterPos++,this.leftPos+=this.spreadWidth,this.setLeft(this.leftPos),this.currentLocationCfi=this.getPageCfi(),this.book.trigger("renderer:pageChanged",this.currentLocationCfi),this.chapterPos):!1
},

EPUBJS.Renderer.prototype.prevPage=function(){
  return this.chapterPos>1?(this.chapterPos--,this.leftPos-=this.spreadWidth,this.setLeft(this.leftPos),this.currentLocationCfi=this.getPageCfi(),this.book.trigger("renderer:pageChanged",this.currentLocationCfi),this.chapterPos):!1
},
...
EPUBJS.Renderer.prototype.setLeft=function(a){
  this.doc.defaultView.scrollTo(a,0)
},

EPUBJS.Renderer.prototype.determineStore=function(){
  return this.book.fromStorage?"filesystem"==this.book.storage.getStorageType()?!1:this.book.store:this.book.contained?this.book.zip:!1
},

EPUBJS.Renderer.prototype.replace=function(a,b,c,d){
  var e=this.doc.querySelectorAll(a),
  ...
  return 0===g?(c(!1),void 0):(f.forEach(function(a){
    b(a,h)
  }.bind(this)),void 0)
},

EPUBJS.Renderer.prototype.replaceWithStored=function(a,b,c,d){
  var e,
  f={},
  g=this.determineStore(),
  h=this.caches[a],
  i=this.book.settings.contentsPath,
  j=b,
  k=function(a,b){
    f[b]=a
  },
  ...
  g&&(h||(h={}),e=_.clone(h),this.replace(a,function(a,b){
    var d=a.getAttribute(j),
    h=EPUBJS.core.resolveUrl(i,d),
    k=function(c){
      a.setAttribute(j,c),
      a.onload=function(){
        b(c,h)
    ...
    h in e?(k(e[h]),f[h]=e[h],delete e[h]):c(g,h,k,a)},l,k))
},

EPUBJS.Renderer.prototype.replaceLinks=function(a){
  var b=this;this.replace("a[href]",function(a,c){
    var d=a.getAttribute("href"),
    e=d.search("://");
    "#"==d[0],
    -1!=e?a.setAttribute("target","_blank"):a.onclick=function(){
      return b.book.goto(d),!1
    },c()
  },a)
},

EPUBJS.Renderer.prototype.page=function(a){
  return a>=1&&a<=this.displayedPages?(this.chapterPos=a,this.leftPos=this.spreadWidth*(a-1),this.setLeft(this.leftPos),!0):!1
},
...
EPUBJS.Renderer.prototype.pageByElement=function(a){
  var b,c;a&&(b=this.leftPos+a.getBoundingClientRect().left,c=Math.floor(b/this.spreadWidth)+1,this.page(c))
},

EPUBJS.Renderer.prototype.beforeDisplay=function(a){
  this.book.triggerHooks("beforeChapterDisplay",a.bind(this),this)
},

EPUBJS.Renderer.prototype.walk=function(a){
  for(var b,a,c,d,e,f=a,g=[f],h=1e4,i=0;!b&&g.length;){
    if(a=g.shift(),this.isElementVisible(a)&&(b=a),!b&&a&&a.childElementCount>0){
      c=a.children,
      d=c.length;
      for(var j=0;d>j;j++)c[j]!=e&&g.push(c[j])
    }
  if(!b&&0==g.length&&f&&null!==f.parentNode&&(g.push(f.parentNode),e=f,f=f.parentNode),i++,i>h){
...
EPUBJS.Renderer.prototype.getPageCfi=function(){
  var a=this.visibileEl;
  return this.visibileEl=this.findFirstVisible(a),this.visibileEl.id||(this.visibileEl.id="EPUBJS-PAGE-"+this.chapterPos),
  this.pageIds[this.chapterPos]=this.visibileEl.id,
  this.epubcfi.generateFragment(this.visibileEl,this.currentChapterCfi)
},

EPUBJS.Renderer.prototype.gotoCfiFragment=function(a){
  var b;_.isString(a)&&(a=this.epubcfi.parse(a)),
  b=this.epubcfi.getElement(a,this.doc),
  this.pageByElement(b)
},

EPUBJS.Renderer.prototype.findFirstVisible=function(a){
  var b,c=a||this.bodyEl;return b=this.walk(c),b?b:a
},

EPUBJS.Renderer.prototype.isElementVisible=function(a){
  var b;return a&&"function"==typeof a.getBoundingClientRect&&(b=a.getBoundingClientRect().left,b>=0&&b<this.spreadWidth)?!0:!1
},

EPUBJS.Renderer.prototype.height=function(){
...
var EPUBJS=EPUBJS||{};EPUBJS.replace={},
...
EPUBJS.replace.svg=function(a,b){
  b.replaceWithStored("image","xlink:href",function(a,b,c){a.getUrl(b).then(c)},a)
},
...
EPUBJS.replace.links=function(a,b,c,d){
  "stylesheet"===d.getAttribute("rel")?EPUBJS.replace.stylesheets(a,b).then(c):a.getUrl(b).then(c)
},

EPUBJS.replace.stylesheets=function(a,b){
  var c=new RSVP.Promise;
  if(a)return a.getText(b).then(function(d){
    EPUBJS.replace.cssUrls(a,b,d).then(function(a){
      var b=window.URL||window.webkitURL||window.mozURL,d=new Blob([a],{type:"text/css"}),e=b.createObjectURL(d);c.resolve(e)
    },
...
EPUBJS.replace.cssUrls=function(a,b,c){
  var d=new RSVP.Promise,e=[],
  f=c.match(/url\(\'?\"?([^\'|^\"]*)\'?\"?\)/g);
  if(a)return f?(f.forEach(function(d){
    var f=EPUBJS.core.resolveUrl(b,d.replace(/url\(|[|\)|\'|\"]/g,""));
    replaced=a.getUrl(f).then(function(a){
      c=c.replace(d,'url("'+a+'")')
    },
    function(a){console.error(a)}),
    e.push(replaced)}),RSVP.all(e).then(function(){d.resolve(c)}),d):(d.resolve(c),d)
},

EPUBJS.Unarchiver=function(a){
  return this.libPath=EPUBJS.filePath,this.zipUrl=a,this.loadLib(),this.urlCache={},this.zipFs=new zip.fs.FS,this.promise
},

EPUBJS.Unarchiver.prototype.loadLib=function(){
  "undefined"==typeof zip&&console.error("Zip lib not loaded"),
  zip.workerScriptsPath=this.libPath
},

EPUBJS.Unarchiver.prototype.openZip=function(a){
  var b=new RSVP.Promise,
  c=this.zipFs;
  return c.importHttpContent(a,!1,function(){
    b.resolve(c)},this.failed),b
},

EPUBJS.Unarchiver.prototype.getXml=function(a){
  return this.getText(a).then(function(a){
    var b=new DOMParser;return b.parseFromString(a,"application/xml")
  })
},
...
EPUBJS.Unarchiver.prototype.getUrl=function(a,b){
  ...
  d=new RSVP.Promise,
  e=this.zipFs.find(a),
  f=window.URL||window.webkitURL||window.mozURL;return e||console.error(a),a in this.urlCache?(d.resolve(this.urlCache[a]),d):(e.getBlob(b||zip.getMimeType(e.name),function(b){
    var e=f.createObjectURL(b);d.resolve(e),c.urlCache[a]=e}),d)
},

EPUBJS.Unarchiver.prototype.getText=function(a){
  var b=new RSVP.Promise,c=this.zipFs.find(a);
  return window.URL||window.webkitURL||window.mozURL,c||console.error(a),c.getText(function(a){b.resolve(a)},null,null,"ISO-8859-1"),b
},

EPUBJS.Unarchiver.prototype.revokeUrl=function(a){
  var b=window.URL||window.webkitURL||window.mozURL,c=unarchiver.urlCache[a];console.log("revoke",c),c&&b.revokeObjectURL(c)
},
...
EPUBJS.Unarchiver.prototype.saveEntryFileToStorage=function(a,b){
  a.getData(new zip.BlobWriter,function(c){
...
/*EPUBJS.Renderer.prototype.getCurrentPage = function(){
    ...
    // console.log(this.currentChapter,this.firstPageOfChapter(this.currentChapter),this.chapterPos);
    return this.firstPageOfChapter(this.currentChapter) + this.chapterPos;

}*/

// Get Current Spin Pos
/*EPUBJS.Renderer.prototype.getCurrentSpin = function(){
    if (!this.isScanningComplete) return -1;
    //console.log(this.currentChapter);
    // console.log(this.currentChapter,this.firstPageOfChapter(this.currentChapter),this.chapterPos);
...
/*EPUBJS.Renderer.prototype.calcNextPage = function(){
  //console.log(this);
    ...
    if (this.book.spine[this.currPageScanned+1]){
        ...
        this.tempIFrame.id ="temporary";
        
        // console.log(this.el.clientWidth,this.el.clientHeight)
        // console.log(this.currPageScanned,this.tempIFrame.src);
        ...
        this.tempIFrame.onload = function(){
            ...
            //bodyEl{font-size:configure[0];font-family:configure[1];line-height:configure[2]};
            //console.log(docEl);
            //console.log(bodyEl);
            
            //configure_reader();

            // $('#temporary').contents().find('p span').contents().unwrap();
            // $('#temporary').contents().find('p').css('margin-bottom','0');
            // Book.setStyle('text-align','left');
            // Book.setStyle('line-height','1.5');
            // $('#temporary').contents().find('p').css('font-size','12px');
            // Book.setStyle('font-size','12px');
                    
            // $('docEl').find('span');
            //var r_config=["12px","georgia",1,1];
            bodyEl.style.fontSize=configure[0];
            bodyEl.style.fontFamily=configure[1];
            bodyEl.style.lineHeight=configure[2];
            //console.log(bodyEl);
            ...
            //console.log("scanNext-pages",render.totalPages,displayedPages,render.book.spine[render.currPageScanned].href);
...
// Calculate spesific address by spinPos
