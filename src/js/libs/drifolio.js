...
$(window).load(function() { // makes sure the whole site is loaded
    // var a = JSON.parse(window.localStorage.getItem('ijog_intro'));
    // try{
    //     if(a!=null){
    //         if(a.meta.status==true){
    //             // if (navigator.appVersion.indexOf("Win")!=-1){
    //             //     var home_p= homePath.replace('C:','')+"/.iNgawi";
    //             // }else{
    //             //     var home_p= homePath+"/.iNgawi";
    //             // }
    //             var home_p= homePath+"/.iNgawi";
    //             isDirectory(home_p+'/'+a.meta.version+'/')
    //             isExist(home_p+'/'+a.meta.version+'.zip')
    //             console.log(dir_status)
    //             if(dir_status==true){
    //                 $('#preloader').html('<object type="text/html" data="'+home_p+'/'+a.meta.version+'/intro.html" style="height:100%;position:relative;width:100%;border:0"></iframe>');
    //                 $('#status').fadeOut();
    //                 if(a.data.time){
    //                     var waktu = parseInt(a.data.time)
    //                     console.log(waktu)
    //                     $('#preloader').delay(waktu).fadeOut('slow');
    //                     $('body').delay(waktu).css({'overflow':'visible'});
    //                 }else{
    ...
    //         }
    //     }
    //     var epus = majax_empty(App.api.url_update+'intro.json','','');
    //     epus.success(function(data){
    //         if(data.meta.code=="200"){
    //             var sValue = JSON.stringify(data);
    //             window.localStorage.setItem('ijog_intro', sValue);
    //             if(data.meta.status==true){
    //                 var link = data.data.download;
    //                 var home_p= homePath+"/.iNgawi";
    //                 isDirectory(home_p+'/'+data.meta.version+'/')
    //                 isExist(home_p+'/'+data.meta.version+'.zip')
    //                 var destination1 = home_p+'/'+data.meta.version+'/';
    //                 var source = home_p+'/'+data.meta.version+'.zip';
    //                 // if (navigator.appVersion.indexOf("Win")!=-1){
    //                 //     var home_p= homePath+"/.iNgawi";
    //                 // }else{
    //                 //     var home_p= homePath+"/.iNgawi";
    //                 // }
    //                 dir('create',destination1)
    //                 dir('create',home_p+'/'+data.meta.version+'/');
    //                 if(dir_status==true){
    //                     $('#preloader').html('<object type="text/html" data="'+home_p+'/'+a.meta.version+'/intro.html" style="height:100%;position:relative;width:100%;border:0"></iframe>');
    //                     $('#status').fadeOut();
    //                     if(a.data.time){
    ...
    //                     }
    //                 }else{
    //                     download(link,function callback(a){
    //                         // $('.second').val(persentation).trigger('change');
    //                         console.log(a)
    //                         if(a==100){
    //                             $('#preloader').hide();
    //                             // console.log(home_p+'/'+data.meta.version+'.zip','',home_p)
                                
    //                             setTimeout(function(){
    //                                 if (navigator.appVersion.indexOf("Win")!=-1){
    //                                   console.log('windows')
    //                                   execute('C:/Windows/unzip.exe -o  -P 123 -d "'+destination1+'" "'+source+'"',function(a){
    //                                     console.log(a);
    //                                   });
    //                                 }else{
    //                                   console.log('osx')
    //                                   execute('unzip -o  -P 123  -d '+destination1+' '+source,function(a){
    //                                     console.log(a)
    //                                   });  
    //                                 }
    //                                 // if (navigator.appVersion.indexOf("Win")!=-1){
    //                                 //     console.log(exec('unzip.exe -o -P '+keyhash+' '+'"'+home_p+'/'+data.meta.version+'.zip'+'"'+' -d '+'"'+home_p+'/'+data.meta.version+'/'+'"'));
    //                                 // }else{
    //                                 //     console.log(exec('unzip -o -P '+keyhash+' '+'"'+home_p+'/'+data.meta.version+'.zip'+'"'+' -d '+'"'+home_p+'/'+data.meta.version+'/'+'"'));  
    //                                 // }
    //                                 setTimeout(function(){
    //                                     // $('#preloader').html('<object type="type/html" data="'+home_p+'/'+data.meta.version+'/intro.html" ></object>');
    //                                 },1000)
    //                             },1000) 
    //                         }
    //                     })
